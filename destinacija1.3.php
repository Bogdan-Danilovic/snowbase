...
<head>
    ...
    <style>
        ...
        .prava-mapa-container {
            ...
            border: 1px solid var(--border-card); box-shadow: 0 20px 50px rgba(0,0,0,0.6);
        ...
        .slope-item:hover { background: var(--surface-hover); border-color: var(--ice); transform: translateX(5px); }

        .scroll-content {
            position: relative; margin-top: 100vh; background: var(--void);
            border-top: 1px solid var(--border-subtle); padding: 60px; z-index: 20;
            box-shadow: 0 -20px 50px rgba(0,0,0,0.9);
        }

        .container-wide { max-width: 1200px; margin: 0 auto; }
        .section-title { font-family: 'Cormorant Garamond', serif; font-size: 2.5rem; margin-bottom: 30px; font-weight: 700; }
        .section-title span { color: var(--ice); }

        .hotel-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 25px; }
        .hotel-card { background: var(--surface); border-radius: 12px; border: 1px solid var(--border-card); overflow: hidden; transition: 0.3s; }
        .hotel-card:hover { border-color: var(--ice); transform: translateY(-5px); }
        ...
        .hotel-price { font-size: 1.4rem; font-weight: 700; border-top: 1px solid var(--border-subtle); padding-top: 12px; margin-top: 12px; }

        .logistika-panel {
            background: var(--surface); padding: 40px; border-radius: 12px; border: 1px solid var(--border-card);
            margin-top: 60px; display: grid; grid-template-columns: 1fr 1fr; gap: 40px;
        }

        .input-group { margin-bottom: 15px; }
        .input-group label { display: block; color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 6px; }
        .input-group input {
            width: 100%; padding: 12px; background: rgba(0,0,0,0.4);
            border: 1px solid var(--border-card); border-radius: 6px; color: #fff; font-size: 0.95rem;
        }
        .input-group input:focus { outline: none; border-color: var(--ice); }

        .btn-calc {
            width: 100%; padding: 12px; background: var(--ice); color: var(--void);
            border: none; border-radius: 6px; font-weight: 600; cursor: pointer; transition: 0.2s;
        }
        .btn-calc:hover { background: #fff; }

        .stat-item { padding: 12px; background: rgba(0,0,0,0.2); border-radius: 8px; margin-bottom: 12px; border: 1px solid var(--border-card); }
        .stat-item p { font-size: 0.8rem; color: var(--text-secondary); }
        .result-box { background: rgba(0, 229, 255, 0.04); padding: 20px; border-radius: 8px; border: 1px solid rgba(0, 229, 255, 0.15); text-align: center; }
    </style>
</head>
<body>
...
<nav>
    ...
    <div class="nav-links">
        ...
        <a href="#smestaj" class="active">Smestaj</a>
        <a href="#logistika" class="active">Logistika</a>
    </div>
</nav>
...
<script>
    // Dinamičko dodavanje pozicije miša za CSS varijable u nav linkovima (Svetlosni efekat menija ostaje)
    ...
    // Funkcija za paljenje i gašenje interaktivnih staza na mapi
    ...
    // Scroll i Reveal efekti za kartice i hero sekciju
    window.addEventListener('scroll', () => {
        ...
        if (hero) {
            if (scrolled > 80) hero.classList.add('scrolled');
    ...
    // Proračun troškova kalkulatora logistike

-- ============================================================================
-- Snowbase — SEED PODACI (bez Zermatt i Cortina d'Ampezzo)
-- Idempotentno: brise sve postojece seed-podatke pre upisivanja novih.
-- `staze_putanje` ostaje netaknuta — nju popunjavas kroz admin.
--
-- Pre pokretanja: schema mora postojati (pokreni `migracija.sql` jednom).
-- ============================================================================
...
DELETE FROM `transport_opcije`;
DELETE FROM `destinacije_slike`;
DELETE FROM `ski_pas_cene`;
DELETE FROM `staze_status`;
DELETE FROM `vreme_prognoza`;
DELETE FROM `vreme_trenutno`;
DELETE FROM `smestaj`;
DELETE FROM `ski_info`;
DELETE FROM `destinacije`;
DELETE FROM `granicni_prelazi`;
...
-- NOTE: `staze_putanje` se popunjava kroz admin/crtanje-staza.php, ne seed-uje se.
